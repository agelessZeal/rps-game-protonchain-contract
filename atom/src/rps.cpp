...
namespace proton
{
  void rps::addplan (const Plan& plan) {
  ...
  void rps::updateplan (const Plan& plan) {
    require_auth(get_self());
  ...
  void rps::removeplan (const uint64_t& plan_index) {
    require_auth(get_self());
  ...
  void rps::process (const uint64_t& max) {
  ...
  void rps::end_subscription(const Subscription& subscription) {
